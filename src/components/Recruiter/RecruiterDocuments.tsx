...
export function RecruiterDocuments() {
    ...
    const stats = {
        ...
        reviewed: documents.filter(d => d.status === 'REVIEWED' || d.status === 'APPROVED').length,
        uploaded: documents.filter(d => d.status === 'UPLOADED').length,
    };

    const filteredDocuments = documents.filter(doc => {
        const fileName = doc.fileRef?.split(/[\\/]/).pop() || 'document';
        const candidateName = doc.requestedFrom?.name || 'Unknown';
        const matchesSearch =
        ...
        <div className="p-8 space-y-6">
                    ...
                    <div>
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                                ...
                                <p className="text-slate-400 mt-1 text-sm">Manage candidate documents and compliances</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-3 gap-6 animate-slide-in" style={{ animationDelay: '100ms' }}>
                {[
                    ...
                    { label: 'Pending Review', value: stats.uploaded, gradient: 'from-blue-500 to-cyan-500', icon: Upload },
                    { label: 'Verified', value: stats.reviewed, gradient: 'from-green-500 to-emerald-500', icon: Sparkles },
                ].map((stat, index) => {
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="glass-dark border-b border-white/10">
                            <tr>
                                ...
                                <th className="px-8 py-5 text-left"><span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Updated</span></th>
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filteredDocuments.map((doc, index) => {
                                ...
                                const statusStyle = (statusColors as any)[doc.status] || { bg: 'bg-slate-500/20', text: 'text-slate-400', border: 'border-slate-500/30' };
                                const typeStyle = (typeColors as any)[doc.type] || { bg: 'bg-slate-500/20', text: 'text-slate-400', border: 'border-slate-500/30' };
                                    ...
                                    >
                                        ...
                                        <td className="px-8 py-5 whitespace-nowrap">
                                            <div className="flex flex-col">
                                                <span className="text-slate-300 font-medium">{doc.requestedFrom?.name || 'Unknown'}</span>
                                                <span className="text-xs text-slate-500">{doc.requestedFrom?.email || ''}</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-5 whitespace-nowrap">
                                            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider badge-glow ${typeStyle.bg} ${typeStyle.text} border ${typeStyle.border}`}>
                                                {doc.type}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-8 py-5 whitespace-nowrap">
                                            <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold uppercase tracking-wider badge-glow ${statusStyle.bg} ${statusStyle.text} border ${statusStyle.border}`}>
                                                <div className={`w-2 h-2 rounded-full ${['REVIEWED', 'APPROVED'].includes(doc.status) ? 'bg-green-400 animate-pulse-glow' : statusStyle.text.replace('text-', 'bg-')}`} />
                                                {doc.status}
                                            </span>
                                        </td>
                    ...
                    {filteredDocuments.length === 0 && (
                        <div className="flex flex-col items-center justify-center p-20 text-slate-500">
                            <File className="w-16 h-16 opacity-20 mb-4" />
